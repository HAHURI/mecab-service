...
# 辞書のアップデート
@app.route('/mecab/v1/update-neologd', methods=['POST'])
def update_neologd():
    args = ['/opt/mecab-ipadic-neologd/bin/install-mecab-ipadic-neologd', '-n', '-y', '-u', '-p', '/usr/local/lib/mecab/dic/mecab-ipadic-neologd']
    try:
        ...
        res = "error"
        status = 400
    return jsonify({'status': status, 'message': messages[0], 'results': [res], 'dict': 'neologd-update'}), status
